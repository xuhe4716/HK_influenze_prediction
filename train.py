...
def main():
    ...
    parser.add_argument('--input_size', type=int, default=3, help='input dimension') # 特征数量 11个特征加自己本身的上一个时刻的ground truth
    ...
    parser.add_argument('--predicted_size', type=int, default=4, help='predicted dimension') # only for seq2seq
    parser.add_argument('--model', type=str, default="Seq2Seq", help='model') # 换成Baseline可以看基模型
    ...
    if args.model == "Seq2Seq":
        Dtr, Val, Dte, m, n =nn_seq_mo(seq_len=args.seq_len, B=batch_size, num=args.predicted_size,
                                       predict_index = args.predict_index,removed_factors = ['weekid','yearid','monthid']) #预测RSV时把这里的RSV 替换成pos_index
    else:
