"""
Weighted GRU model train and predict
Yearly rolling
Author: He
"""
...
from Model.GRUFcnModel import GRUDataset, GRUModel, GRUWeightedTrain
...
model_name = f'GRUFcn_v3_nontuning_rolling_v2_{predict_index}_{data_folder.split("/")[1]}'
############################################### modeling ##########################################
study = joblib.load("./model_hyperparam/Separate_GRU.pkl") #load
...
def one_bootstrap(i, df, test_size, total_pred_horizon = 5):
    ...
    data_deal1 = GRUDataset(sequence_length=seq_length1,
                            batch_size=batch_size1,
                            pred_stamp=pred_stamp)
    train_dataloader1, val_datadict1, test_datadict1 = data_deal1.get_train_val_test_dataset(df, test_size = test_size, sample_rate=None)
    rate_max1, rate_min1 = data_deal1.get_rate_scaler()

    x_test1, y_test1 = test_datadict1['x_data'], test_datadict1['y_data']
    model1 = GRUModel(input_dim, output_dim, sequence_dim=seq_length1,
                      mid_dim=mid_dim1, hidden_layers=hidden_layers1, dropout_rate=dropout_rate1, bidirectional=False, MCDropout = None)
    model_train1 = GRUWeightedTrain(model1)
    model_train1.train(train_dataloader1, val_datadict1, num_epochs = 50, lr=lr1, early_stopping=None, verboose=2)
    # get predict result
    _, y_test_pred1 = model_train1.predict_xy(x_test1, y_test1)

    ## week 8 : model building
    # torch.manual_seed(i)
    # random.seed(i)
    # np.random.seed(i)
    input_dim = df.shape[1]
    ...
    output_dim = pred_stamp
    data_deal4 = GRUDataset(sequence_length=seq_length2,
                            batch_size=batch_size2,
                            pred_stamp=pred_stamp)
    train_dataloader4, val_datadict4, test_datadict4 = data_deal4.get_train_val_test_dataset(df, test_size = test_size, sample_rate=None)
    rate_max4, rate_min4 = data_deal4.get_rate_scaler()
    model4 = GRUModel(input_dim, output_dim, sequence_dim=seq_length2,
                      mid_dim=mid_dim2, hidden_layers=hidden_layers2, dropout_rate=dropout_rate2, bidirectional=False, MCDropout = None)
    model_train4 = GRUWeightedTrain(model4)
    model_train4.train(train_dataloader4, val_datadict4, num_epochs = 50, lr=lr2, early_stopping=None, verboose=2)
    # delay one day
