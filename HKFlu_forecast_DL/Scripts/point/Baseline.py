...
from Model.BaselineModel import LstmDataset, BaselineModel, LstmWeightedTrain, LstmTrain
from data import DataTool
import argparse
############################################### data preparation ##########################################
mode = 'test8'
test_start_date = '2009-11-01'
test_end_date = None#'2016-06-30'
parser = argparse.ArgumentParser()
parser.add_argument('--data', type=str, default='weather_data', help='data folder')
parser.add_argument('--predict_index', type=str, default='ILI', help='predict feature name') # 预测RSV时把这里的pos_index替换成RSV
...
col_list = ['rate']
dr = DataTool()
df_o = dr.data_output(path, col_list, mode = 'true')
df = copy.deepcopy(df_o)
model_name = f'Baseline_v3_nontuning_rolling_v2_{predict_index}_{data_folder.split("/")[1]}'
############################################### modeling ##########################################
'''
...
'''
seq_length1 = 14 #trial.params['seq_length1']
mid_dim1 = 72 #trial.params['mid_dim1']
hidden_layers1 = 1 #trial.params['hidden_layers1']
dropout_rate1 = 0.3 #trial.params['dropout_rate1']
lr1 = 0.02 #trial.params["lr1"]
batch_size1 = 6 #trial.params["batch_size1"]

seq_length2 = 14 #trial.params['seq_length2']
mid_dim2 = 72 #trial.params['mid_dim2']
hidden_layers2 = 1 #trial.params['hidden_layers2']
dropout_rate2 = 0.1 #trial.params['dropout_rate2']
lr2 = 0.002 #trial.params["lr2"]
batch_size2 = 6 #trial.params["batch_size2"]
pred_stamp = 9
...
def one_bootstrap(i, df, test_size, total_pred_horizon = 5):
    ...
    model1 = BaselineModel(input_dim, output_dim, sequence_dim=seq_length1,
                  mid_dim=mid_dim1, hidden_lstm_layers=hidden_layers1, num_directions = 1)
    ...
    model4 = BaselineModel(input_dim, output_dim, sequence_dim=seq_length2,
                    mid_dim=mid_dim2, hidden_lstm_layers=hidden_layers2,  num_directions = 1)
...
def one_rolling(df, test_start, test_end, pred_horizon, exp_mode = True, bootstrap_times = 100, random_state = None):
    ...
    re_test = dr.origin_re_output(df_test, left_len=0, pred_len = pred_horizon, exp_mode=False)
