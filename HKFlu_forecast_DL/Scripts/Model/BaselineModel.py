...
class BaselineModel(nn.Module):
    def __init__(self, input_dim, output_dim, sequence_dim, mid_dim=8, hidden_lstm_layers = 3,num_directions = 1):
        ...
        self.num_layers = hidden_lstm_layers
        self.output_size = output_dim
        #self.device = device

    def forward(self, input_seq):
        if self.output_size == 1:
            pred = input_seq[:, -2, 0].unsqueeze(-1)
            return pred
        else:
            predictions = []

            # Iterate over the last 'output_size' elements
            for i in range(-4, -12,-1):
                # Extract the i-th last element, note that indexing starts at 1 hence `-(i+1)`
                pred = input_seq[:, i, 0].unsqueeze(-1)
                predictions.append(pred)

            # Concatenate all predictions along dimension 1
