...
class LstmModel(nn.Module):
    def __init__(self, input_dim, output_dim, sequence_dim, mid_dim=8, hidden_lstm_layers = 3, dropout_rate = 0.5, num_directions = 1, MCDropout = 0.5):
        ...
        if self.MCDropout is not None:
            self.fcn = nn.Sequential(
                nn.Linear(mid_dim * self.num_directions, mid_dim * 4),
                nn.ReLU(),
                nn.Dropout(MCDropout),
                nn.Linear(mid_dim * 4, mid_dim * 2),
                nn.ReLU(),
                nn.Dropout(MCDropout),
                nn.Linear(mid_dim * 2, output_dim)
            )
        else:
            self.fcn = nn.Sequential(
                nn.Linear(mid_dim * self.num_directions, mid_dim * 4),
    ...
    def forward(self, input):
        ...
        h_0 = torch.zeros(self.num_directions * self.hidden_lstm_layers, batch_size, self.hidden_size, device=input.device)
        c_0 = torch.zeros(self.num_directions * self.hidden_lstm_layers, batch_size, self.hidden_size, device=input.device)
        output_y, _ = self.lstm(input, (h_0, c_0))
        if self.MCDropout is not None:
            output_y = self.MC_dropout(output_y.clone())
        output_y = output_y[:, -1, :]  # Taking the last time step output
        pred_y = self.fcn(output_y)  # Pass the last output to the FCN module
